...
<body>
    ...
    <div class="container-fluid page-header py-5 mb-5 wow fadeIn" data-wow-delay="0.1s">
        <div class="container text-center py-5 mt-4">
            <h1 class="display-2 text-white mb-3 animated slideInDown">{{__('menu.Our Web3 Game')}}</h1>
            <nav aria-label="breadcrumb animated slideInDown">
                <ol class="breadcrumb justify-content-center mb-0">
                    <li class="breadcrumb-item"><a href="{{url('/')}}">{{__('menu.Home')}}</a></li>
                    <li class="breadcrumb-item" aria-current="page">{{__('menu.Our Web3 Game')}}</li>
                </ol>
            </nav>
        </div>
    </div>
        ...
        <div class="container pb-5">
            <div class="row g-5 align-items-center mb-5">
                <div class="col-md-6 wow fadeIn" data-wow-delay="0.3s">
                    <img class="img-fluid w-100" src="{{url('uploads/images/guide/'.$guide->image_major)}}" alt="" style="height:400px" >
                </div>
                <div class="col-md-6 wow fadeIn" data-wow-delay="0.5s" >

                    <h5 class="mb-3">{{__('menu.Name Game')}}</h5>
                    <p class="mb-4">{!!app()->getLocale() == "en" ? $guide->en_name : $guide->ar_name!!}</p>
                    <h5 class="mb-3">{{__('menu.Glance Game')}}</h5>
                    <p class="mb-4">{!!app()->getLocale() == "en" ? $guide->en_glance : $guide->ar_glance!!}</p>
                    <h5 class="mb-3">{{__('menu.Registration mechanism')}}</h5>
                    <p class="mb-4">{!!app()->getLocale() == "en" ? $guide->en_registration_mechanism : $guide->ar_registration_mechanism!!}</p>
                    <h5 class="mb-3">{{__('menu.Registration play')}}</h5>
                    <p class="mb-4">{!!app()->getLocale() == "en" ? $guide->en_play_mechanism : $guide->ar_play_mechanism!!}</p>
         
                    <div class="d-flex">
                    <a class="btn btn-lg-square btn-primary me-2" href="{{$guide->tele1}}"><i class="fab fa-telegram"></i></a>

                        @if($guide->tele2 ==NUll)
                        @else
                        <a class="btn btn-lg-square btn-primary me-2" href="{{$guide->tele2}}"><i class="fab fa-telegram"></i></a>
                        @endif
                        @if($guide->tele3 ==NUll)
                        @else
                        <a class="btn btn-lg-square btn-primary me-2" href="{{$guide->tele3}}"><i class="fab fa-telegram"></i></a>
                        @endif
                        @if($guide->tele4 ==NUll)
                        @else
                        <a class="btn btn-lg-square btn-primary me-2" href="{{$guide->tele4}}"><i class="fab fa-telegram"></i></a>
                        @endif
                    </div>
                    <br>
                    <div class="d-flex">
                        <a class="btn btn-lg-square btn-primary me-2" href="{{$guide->link}}"><i class="fa fa-link"></i></a>
                        <a class="btn btn-lg-square btn-primary me-2" href="{{$guide->discord}}"><i class="fab fa-discord"></i></a>
                        <a class="btn btn-lg-square btn-primary me-2" href="{{$guide->twitter}}"><i class="fas fa-times"></i></a>
                    </div>
                </div>
            </div>
